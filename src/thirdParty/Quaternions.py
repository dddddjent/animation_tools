...
class Quaternions:
    ...
    def repeat(self, **kwargs):
        return Quaternions(self.qs.repeat(**kwargs))
    ...
    def euler(self, order='xyz'):
        ...
        if order == 'xyz':
            es[..., 0] = np.arctan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1**2 + q2**2))
            es[..., 1] = np.arcsin((2 * (q0 * q2 - q3 * q1)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2**2 + q3**2))
        elif order == 'yzx':
            es[..., 0] = np.arctan2(2 * (q0 * q2 + q1 * q3), 1 - 2 * (q2**2 + q3**2))
            es[..., 1] = np.arcsin((2 * (q0 * q3 - q1 * q2)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1**2 + q3**2))
        elif order == 'zxy':
            es[..., 0] = np.arctan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q3**2 + q1**2))
            es[..., 1] = np.arcsin((2 * (q0 * q1 - q2 * q3)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q2 + q1 * q3), 1 - 2 * (q2**2 + q1**2))
        elif order == 'xzy':
            es[..., 0] = np.arctan2(2 * (q0 * q1 - q2 * q3), 1 - 2 * (q1**2 + q3**2))
            es[..., 1] = np.arcsin((2 * (q0 * q3 + q1 * q2)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q2 - q1 * q3), 1 - 2 * (q2**2 + q3**2))
        elif order == 'yxz':
            es[..., 0] = np.arctan2(2 * (q0 * q2 - q1 * q3), 1 - 2 * (q2**2 + q1**2))
            es[..., 1] = np.arcsin((2 * (q0 * q1 + q2 * q3)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q3 - q1 * q2), 1 - 2 * (q3**2 + q1**2))
        elif order == 'zyx':
            es[..., 0] = np.arctan2(2 * (q0 * q3 - q1 * q2), 1 - 2 * (q3**2 + q2**2))
            es[..., 1] = np.arcsin((2 * (q0 * q2 + q1 * q3)).clip(-1, 1))
            es[..., 2] = np.arctan2(2 * (q0 * q1 - q2 * q3), 1 - 2 * (q1**2 + q2**2))
        else:
    ...
    def identity(N=None):
        if N is None:
            ...
            return Quaternions(np.array([[1, 0, 0, 0]] * N))
