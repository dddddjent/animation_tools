...
def moving_average(x, w):
    w_np = np.array(w)
    return np.convolve(x, w_np, 'valid') / w_np.sum()
...
#        jmatch = re.match("\s*JOINT\s+(\w+)", line)
        ...
        if dmatch:
            ...
            if   channels == 3:
                ...
                # # rotations[fi, 0, :], rotations[fi, 1, :] = rotations[fi, 1, :], rotations[fi, 0, :]
                # eula_r0 = Rotation.from_euler("zyx", rotations[fi, 0, :] ,degrees=True)
                # eula_r1 = Rotation.from_euler("zyx", rotations[fi, 1, :] ,degrees=True)
                # rotation_r0r1 = np.dot(eula_r1.as_matrix(), eula_r0.as_matrix())
                # new_rotation =  Rotation.from_matrix(rotation_r0r1).as_euler("zyx",degrees=True)
                # rotations[fi, 0, :] = new_rotation
    ...
    # for k in range(1, positions.shape[1]):
    #     for d in range(3):
